...
class IndicatorsClient:
    def __init__(self, taapi_io_apikey: str = None):
        self.taapi_key = taapi_io_apikey
        self.ta_db_path = join(dirname(abspath(__file__)), "ta_db.json")

    def simple_indicator(self, pair_price: float, alert: dict) -> tuple[bool, float]:
        """
        Handles the following simple price movement indicators:
    ...
    def get_pair_price(self, token_pair, retry_delay: int = 2, maximum_retries: int = 5, _try: int = 1) -> float:
        """
